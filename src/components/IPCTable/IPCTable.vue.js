...
export default {
  buildCols() {
    let rs = this.buildColsWithCheckList(this.pColsData, this.colsForCheckList);
    console.info(rs, this.colsForCheckList, this.pColsData, this.rowsData);
    ...
    let cols = colArr.map(col => {
      if (!col.width) {
        ...
        let maxWidth = Math.max(
          ...rows.map(it => {
            if (it) {
              console.info("------------------------------", it, it.toString());
              return strlen(it.toString()) * PER_PIX_FOR_NUMBER + minxWidth;
            } else {
              return minxWidth;
            }
          })
